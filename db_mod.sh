...
do
	...
	do
		...
		db_mod_table=$(mysql -e "use $DB;select concat(table_name,', ',max(create_time)) from information_schema.tables where table_schema = database();" | grep -v "\----" | grep -v "max(update_time)" | grep -v "table_name")
		...
		then
			...
			echo -e "Database - '\033[1m$DB\033[0m', last created table and creation time - '\033[1m$db_create_table\033[0m', $date_diff_create days ago, and no modification occured."
			echo ""
		else
